...
function createOrderCard(order) {
    const statusConfig = {
        'placed': { color: 'text-slate-500', bg: 'bg-slate-100', icon: 'fa-box', label: 'Placed' },
    ...
    // Invoice Buttons (Show if invoiceId exists)
    if (order.invoiceId) {
        const invId = typeof order.invoiceId === 'object' ? order.invoiceId._id : order.invoiceId;
        actionButtons += `
            <div class="flex gap-2">
                <button onclick="viewInvoice('${invId}')" class="bg-indigo-50 text-indigo-600 px-4 py-2.5 rounded-xl font-bold hover:bg-indigo-100 transition-all text-xs flex items-center gap-2 border border-indigo-100"><i class="fas fa-eye"></i> View</button>
                <button onclick="downloadInvoice('${invId}')" class="bg-indigo-500 text-white px-4 py-2.5 rounded-xl font-bold hover:bg-indigo-600 transition-all text-xs flex items-center gap-2 shadow-sm"><i class="fas fa-download"></i> Download Invoice</button>
            </div>
        `;
    }
    ...
    const paymentLabel = order.paymentMethod?.toUpperCase() || 'COD';

    // Address Snippet with Pincode
    const addr = order.shippingAddress || {};
    const addressSnippet = addr.city ? `${addr.city}, ${addr.state}${addr.pincode ? ` - ${addr.pincode}` : ''}` : 'Standard Shipping';
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 hover:shadow-md transition-all group animate-fade-in">
            <div class="flex flex-col md:flex-row gap-6">
                <div class="w-full md:w-32 h-32 rounded-2xl bg-slate-50 overflow-hidden flex-shrink-0">
                    <img src="${itemImg}" alt="${firstItem.name}" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/400x400?text=GenziKart'">
                </div>
                ...
                <div class="flex-1">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 mt-1">Shipping to: <span class="text-slate-600 font-bold">${addressSnippet}</span></p>
                            <p class="text-[10px] text-slate-400 mt-1">${order.items.length > 1 ? `+ ${order.items.length - 1} more items` : ''}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xl font-black text-slate-900">₹${order.totalAmount || order.total}</p>
                    ...
                    <div class="flex flex-wrap items-center gap-4 mt-4 py-3 border-y border-slate-50">
                        ...
                        <span class="text-xs font-bold text-slate-400"><i class="fas fa-wallet mr-1"></i> ${paymentLabel}</span>
                    </div>
...
function viewInvoice(invoiceId) {
    ...
    window.open(`${API_BASE_URL}/invoices/${invoiceId}/view?token=${token}`, '_blank');
}

function downloadInvoice(invoiceId) {
    const token = localStorage.getItem('token');
    window.open(`${API_BASE_URL}/invoices/${invoiceId}/download?token=${token}`, '_blank');
}

function downloadDigital(id) {
    showToast('Preparing download...', 'success');
    const token = localStorage.getItem('token');
    window.location.href = `${API_BASE_URL}/orders/${id}/download?token=${token}`;
}
